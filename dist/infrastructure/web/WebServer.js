...
class WebServer {
    constructor(repository, controller, adapter, dispatch, port = 3000, orchestrator) {
    ...
    handle(req, res) {
        ...
        if (method === 'GET' && url === '/api/orch')
            return this.apiOrchGet(res);
        if (method === 'POST' && url === '/api/orch')
            return this.apiOrchPost(req, res);
    ...
    apiOrchGet(res) {
        ...
        res.end(JSON.stringify({
            available: true,
            autonomous: this.orchestrator.autonomousMode,
            phase: state.phase,
        ...
        req.on('end', () => {
            ...
            try {
                ...
                switch (payload.action) {
                    ...
                    case 'pause_bot':
                        if (payload.botId)
                            this.orchestrator.pauseBot(payload.botId, payload.durationMs ?? 300000);
                        break;
                    case 'resume_bot':
                        if (payload.botId)
                            this.orchestrator.resumeBot(payload.botId);
                        break;
                    case 'set_phase':
                        if (payload.phase)
                            this.orchestrator.setPhase(payload.phase);
                        break;
