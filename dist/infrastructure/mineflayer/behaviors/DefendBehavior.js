        ...
        const tick = () => {
            ...
            if (defendState === 'attacking' && combatOrigin) {
                ...
                if (drift > MAX_CHASE_DIST) {
                    console.warn(`[${(0, utils_1.ts)()}][Defend] ${domainBot.username}: drifted ${Math.floor(drift)}m from origin — disengaging`);
                    lastThreatId = null;
                    combatOrigin = null;
                    defendState = 'idle';
            ...
            if (primaryMob) {
                if (defendState !== 'attacking' || lastThreatId !== primaryMob.id) {
                    ...
                    if (prev !== 'attacking')
                        combatOrigin = mfBot.entity.position.clone();
                    defendState = 'attacking';
