...
export class CommandListener {
  ...
  attachChatListeners(): void {
    // Only attach to bots that don't have a listener yet (guard against double-attach on spawn)
    this.repository.findAll().forEach(domainBot => {
      const mfBot = domainBot.handle as MineflayerBot | null;
      if (!mfBot) return;
      if ((mfBot as unknown as { _swarmChatAttached?: boolean })._swarmChatAttached) return;
      (mfBot as unknown as { _swarmChatAttached?: boolean })._swarmChatAttached = true;
      mfBot.on('chat', (username: string, message: string) => {
        if (username !== config.master.username) return;
        // Deduplicate: many bots hear the same message simultaneously
        const now = Date.now();
        if (message === this.lastChat.input && now - this.lastChat.time < 1000) return;
        this.lastChat = { input: message, time: now };
        console.log(`[CommandListener] Chat command from ${username}: ${message}`);
  ...
  private parseTarget(input: string): [BotTarget, string] {
    if (!input.startsWith('@')) return [undefined, input];
    ...
    if (targetStr === 'all') return [undefined, rest];
    ...
    if (rangeMatch) {
      ...
      const target: string[] = all.filter(name => {
    ...
    if (groupMembers !== undefined) return [groupMembers, rest];
  ...
  dispatch(input: string): void {
    ...
    switch (cmd.toLowerCase()) {
      ...
      case 'move': {
        ...
        if ([x, y, z].some(isNaN)) { console.log('Usage: move <x> <y> <z>'); break; }
      ...
      case 'follow': {
        const [t] = args;
        if (!t) { console.log('Usage: follow <player>'); break; }
      ...
      case 'say': {
        ...
        if (!message) { console.log('Usage: say <message>'); break; }
      ...
      case 'attack': {
        const [t] = args;
        if (!t) { console.log('Usage: attack <player>'); break; }
        this.controller.attackAll(t, target);
        break;
      }

      case 'pvp': {
        if (args.length === 0) { console.log('Usage: pvp <player> [player2 ...]'); break; }
      ...
      case 'guard': {
        ...
        if ([x, y, z].some(isNaN)) { console.log('Usage: guard <x> <y> <z> [radius]'); break; }
      ...
      case 'farm': {
        ...
        if (isNaN(rawCX) || isNaN(rawCZ)) { console.log('Usage: farm <centerX> <centerZ> [radius]'); break; }
        this.controller.farmAll(rawCX, rawCZ, isNaN(rawR) ? 30 : rawR, target);
        break;
      }

      case 'explore': {
        const dir = (args[0] ?? 'auto') as 'north'|'south'|'east'|'west'|'auto';
      ...
      case 'avoid': {
        ...
        if (players.length === 0) { console.log('Usage: avoid <player> [player2] [--radius N]'); break; }
      ...
      case 'build': {
        ...
        const schematicPath = path.resolve(process.cwd(), file);
        this.controller.buildAll(schematicPath, bx, by, bz, target).catch(err =>
          console.error(`[CommandListener] Build failed: ${err.message}`),
        );
        break;
      }
      ...
      case 'equip': {
        const [itemName] = args;
        if (!itemName) { console.log('Usage: equip <itemName>'); break; }
      ...
      case 'group': {
        ...
        switch (sub) {
          case 'create':
            if (!name) { console.log('Usage: group create <name>'); break; }
            this.groups.create(name);
            break;
          case 'delete':
            if (!name) { console.log('Usage: group delete <name>'); break; }
            this.groups.delete(name);
            break;
          case 'add':
            if (!name || rest.length === 0) { console.log('Usage: group add <name> <bot1> [bot2 ...]'); break; }
            this.groups.add(name, ...rest);
            break;
          case 'remove':
            if (!name || rest.length === 0) { console.log('Usage: group remove <name> <bot1> [bot2 ...]'); break; }
          ...
          case 'list':
            ...
            break;
          case 'members':
            if (!name) { console.log('Usage: group members <name>'); break; }
      ...
      case 'friend': {
        ...
        if (sub === 'add' && player)    { this.controller.relations.addFriend(player); break; }
        if (sub === 'remove' && player) { this.controller.relations.remove(player); break; }
        if (sub === 'list') { console.log('Friends:', this.controller.relations.getFriends().join(', ') || '—'); break; }
      ...
      case 'enemy': {
        const [sub, player] = args;
        if (sub === 'add' && player)    { this.controller.relations.addEnemy(player); break; }
        if (sub === 'remove' && player) { this.controller.relations.remove(player); break; }
        if (sub === 'list') { console.log('Enemies:', this.controller.relations.getEnemies().join(', ') || '—'); break; }
      ...
      case 'neutral': {
        ...
        this.controller.relations.setNeutralBehavior(mode as 'ignore' | 'attack' | 'armed');
        break;
      }
      ...
      case 'spawn': {
        ...
        if (isNaN(count) || count <= 0) { console.log('Usage: spawn <count>'); break; }
        this.botManager.spawnMore(count).then(() =>
...
function loadTargetFile(filePath: string): string[] {
